...
def runTest(table, thread, total_ops=100, initial_capacity=23, prefill=25, reads=0, inserts=0, upserts=0):
	...
	output_file = "results/test2/"+timestamp+"/"+table+"_"+thread+"_threads_"+reads+"_"+inserts+"_"+upserts
...
if __name__ == '__main__':
	...
	if(not os.path.isdir("results")):
		os.mkdir("results")
	if(not os.path.isdir("results/test2")):
		os.mkdir("results/test2")
	os.mkdir("results/test2/"+timestamp)
	...
	for table in tables:
		for thread in threads:
			...
			runTest(table, thread, total_ops=100, initial_capacity=25, prefill=50, reads = 90, inserts=10)
