...
def runTest(table, thread, total_ops=100, initial_capacity=23, prefill=25, reads=0, inserts=0, upserts=0):
	...
	output_file = "results/test1/"+timestamp+"/"+table+"_"+thread+"_threads_"+reads+"_"+inserts+"_"+upserts
...
if __name__ == '__main__':
	...
	if(not os.path.isdir("results")):
		os.mkdir("results")
	if(not os.path.isdir("results/test1")):
		os.mkdir("results/test1")
	os.mkdir("results/test1/"+timestamp)
	
	n_threads = multiprocessing.cpu_count()
	n_points = 15
	tables = ["random", "random2"]
